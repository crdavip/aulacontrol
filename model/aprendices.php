...
class Aprendices extends ConnPDO
{
  ...
  public function removeTrainee($idTrainee, $idSheet)
  {
    $conn = $this->getConn();
    $conn->beginTransaction();
    try {
      // Elimina el aprendiz de la tabla aprendices
      $sqlDelete = "DELETE FROM aprendices WHERE idAprendices = ? AND idFicha = ?";
      $stmtDelete = $conn->prepare($sqlDelete);
      $stmtDelete->execute([$idTrainee, $idSheet]);

      // Actualiza el conteo de aprendices en la tabla ficha
      $sqlUpdate = "UPDATE ficha 
                      SET aprendices = (SELECT COUNT(*) FROM aprendices WHERE idFicha = ficha.idFicha)
                      WHERE idFicha = ?";
      $stmtUpdate = $conn->prepare($sqlUpdate);
      $stmtUpdate->execute([$idSheet]);
      $conn->commit();
      // Fin transacción
      return [
        'success' => true,
      ...
      return [
        'success' => false,
        'message' => 'Error al eliminar aprendiz: ' . $e->getMessage()
      ];
    }
  }
}